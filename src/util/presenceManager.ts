...
import { loadStrings } from "./langManager";
...
export async function updatePresences() {
	...
	Promise.all(
		...
		const presenceNamesToUpdate = presencesToUpdate.map(p => p.name),
			updatedPresences = presences.filter(p => presenceNamesToUpdate.includes(p.metadata.service));

		updatedPresences.forEach(async p => {
				await initPresenceLanguages(p);

				const settingName = `pSettings_${p.metadata.service}`,
					settings = (await getStorage("local", settingName))[settingName];

				if (settings) {
					const langSettings = settings.find(s => typeof s.multiLanguage !== "undefined");

					if (langSettings && langSettings.values) {
						const langs = langSettings.values.map(v => v.value);

						await Promise.all(
							langs.map(lang => loadStrings(lang, true))
						);
					}
				}
		});
