	...
	export default {
		...
		computed: {
			infoMessage() {
				...
				const regExp = /(\*.*?\*)/g,
					notConnectedString = "popup.info.notConnected.message";

				let msg = this.$t(notConnectedString);

				if (!regExp.test(msg)) {
					msg = this.$t(notConnectedString, this.$root.$options.i18n.fallbackLocale);
				}

				const match = msg.match(regExp)[0];
				msg = msg.replace(
					match,
